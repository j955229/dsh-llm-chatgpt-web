...
async function imageContent(block, attachments, signal) {
...
function flattenResult(blocks) {
    return blocks.map(block => {
        ...
        if (block.type === 'tool-result')
            return flattenResult(block.content);
        if (block.type === 'image')
            return `[image ${block.attachment.mediaType} ${block.attachment.width}x${block.attachment.height}]`;
        return '';
    }).join('\n');
}
async function serializeMessage(message, attachments, currentTurnId, signal) {
    ...
    if (message.role === 'assistant') {
        ...
        for (const block of message.content) {
            if (block.type === 'text')
                visible.push({ type: 'output_text', text: block.text });
            else if (block.type === 'reasoning')
                result.push({ type: 'reasoning', id: `rs_${message.id}`, summary: [{ type: 'summary_text', text: block.text }] });
            else if (block.type === 'tool-call')
                result.push({ type: 'function_call', id: `fc_${block.id}`, call_id: block.id, name: block.name, arguments: block.arguments });
        }
        if (visible.length)
            result.unshift({ type: 'message', id: message.id, role: 'assistant', content: visible });
        return result;
    }
    ...
    for (const block of message.content) {
        if (block.type === 'text' || block.type === 'reasoning')
            ...
            content.push(await imageContent(block, attachments, signal));
        else if (block.type === 'tool-result')
            content.push({ type: 'input_text', text: flattenResult([block]) });
    }
    return [{ type: 'message', id: message.id, role, content, ...metadata }];
}
export async function serializeRequest(options) {

...
export async function* decodeSse(stream, signal) {
...
export async function* responsesToChunks(stream, signal) {
    const open = new Map();
    let nextIndex = 0;
    let terminal = false;
    let sawTool = false;
    const keyFor = (event) => String(event.item_id ?? event.item?.id ?? event.output_index ?? 'default');
    const start = (event, kind, id, name) => {
    ...
    const close = function* (entry) {
        let block;
        if (entry.kind === 'text')
            block = { type: 'text', text: entry.text };
        else if (entry.kind === 'reasoning')
            block = { type: 'reasoning', text: entry.text };
        else
            block = { type: 'tool-call', id: entry.id ?? '', name: entry.name ?? '', arguments: entry.text };
    ...
    for await (const record of decodeSse(stream, signal)) {
        ...
        try {
            ...
            throw new Error(malformedSseMessage(record), { cause });
        }
    ...
    if (!terminal)
        throw new Error('ChatGPT Web SSE stream ended before a terminal response event.');
}

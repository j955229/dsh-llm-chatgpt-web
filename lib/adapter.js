...
function base(value) { return value.replace(/\/+$/, ''); }
function isContextOverflowFailure(failure) {
...
function upstreamFailureCode(status, failure) {
...
export class ChatGptWebAdapter extends LlmAdapter {
    ...
    async resolveModel(provider, model, signal) {
        if (provider !== PROVIDER)
            return Promise.reject(new LlmError(`Unsupported provider: ${provider}`, 'UNSUPPORTED_PROVIDER'));
        signal?.throwIfAborted();
        let route;
        try {
            route = routeFor(model);
        }
        catch (cause) {
            if (signal?.aborted)
                throw signal.reason;
    ...
    async assertToolMode(signal) {
        let health;
        try {
            health = await readBridgeHealth(base(this.options.baseURL), this.fetchImpl, signal, attributionHeaders());
        }
        catch (cause) {
            throw new LlmError('無法連線到 codex-chatgpt-web bridge（預設 127.0.0.1:17841）。', 'UPSTREAM_OFFLINE', { cause: safeCause(cause) });
        }
        const problem = toolModeProblem(health);
        if (problem === 'wrong-service')
            throw new LlmError('17841 連接埠上的服務不是 codex-chatgpt-web。', 'UPSTREAM_INVALID');
    ...
    async *stream(options) {
        ...
        if (!options.sessionId)
            throw new LlmError('ChatGPT Web adapter requires options.sessionId.', 'MISSING_SESSION_ID');
        try {
            routeFor(options.model);
        }
        catch (cause) {
        ...
        if (!compaction && options.tools?.length)
            await this.assertToolMode(options.signal);
        ...
        if (!response.ok) {
            ...
            throw new LlmError(`codex-chatgpt-web request failed: ${formatUpstreamFailure(failure)}`, upstreamFailureCode(response.status, failure), { status: response.status });
        }
        if (!response.body || !response.headers.get('content-type')?.toLowerCase().includes('text/event-stream')) {
            throw new LlmError('codex-chatgpt-web did not return an SSE stream.', 'INVALID_RESPONSE');
        }
        try {
            for await (const chunk of responsesToChunks(response.body, options.signal))
                yield chunk;
        }
        catch (cause) {
            if (options.signal?.aborted)
            ...
            const failure = cause instanceof UpstreamSseError && cause.code
                ...
                : { message };
            throw new LlmError(`codex-chatgpt-web:\n${message}`, isContextOverflowFailure(failure) ? CONTEXT_WINDOW_EXCEEDED_CODE : 'STREAM_ERROR', { cause: safeCause(cause) });
        }
    }
}

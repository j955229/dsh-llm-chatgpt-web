...
export class ChatGptWebAdapter extends LlmAdapter {
    ...
    async resolveModel(provider, model, signal) {
        ...
        try {
            return resolvedModel(await this.catalog.require(model, signal));
        }
        catch (cause) {
            if (signal?.aborted)
                throw signal.reason;
    ...
    async assertToolMode(signal) {
        ...
        try {
            ...
            throw new LlmError('無法連線到 codex-chatgpt-web bridge（預設 127.0.0.1:17841）。', 'UPSTREAM_OFFLINE', { cause });
        }
    ...
    async *stream(options) {
        ...
        if (options.purpose === 'compaction')
            throw new LlmError('ChatGPT Web adapter 不支援 DSH compaction 請求；避免把壓縮回合混入正常 ChatGPT thread。', 'UNSUPPORTED_PURPOSE');
        if (!options.sessionId)
            throw new LlmError('ChatGPT Web adapter requires options.sessionId.', 'MISSING_SESSION_ID');
        try {
            await this.catalog.require(options.model, options.signal);
        }
        catch (cause) {
